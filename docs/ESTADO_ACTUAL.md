...
El `README.md` quedó **desalineado** con la estructura real del repo.

Sigue mostrando comandos como:

- `ruby 00.rb`
- `ruby practicas/mind-reader.rb`

Pero hoy las rutas reales son:

- `examples/lecciones/00.rb`
- `examples/practicas/mind-reader.rb`

Además, el README no presenta a `bin/run` como interfaz principal, cuando en la práctica hoy es la forma más prolija de ejecutar ejemplos.

### Documentación en `docs/`

En el estado actual del working tree, `docs/` contiene solo este archivo y además aparece como contenido nuevo sin trackear.
...
- documentación sincronizada con la estructura real;
...
1. **Actualizar el README** con rutas reales o usando `bin/run` como interfaz principal.
2. **Definir qué hacer con `examples/template/`**: o se confirma su eliminación en Git, o se restaura y se documenta correctamente.
3. **Aclarar el alcance real del repo**: playground educativo de Ruby, no app consolidada.

### Prioridad media

4. Agregar una guía corta de ejecución por grupo.
5. Documentar dependencias especiales como `graphql` y `tk`.
6. Revisar `docker-compose.yml` y decidir si el mapeo de puertos `3000:9292` sigue teniendo sentido.
...
El siguiente paso lógico, de verdad, sería ordenar dos cosas primero:

1. **alinear documentación con la estructura real**;
2. **resolver el estado Git de `examples/template/`**.

Después de eso recién conviene abrir una segunda tanda de docs más operativas, como por ejemplo:

- `docs/COMO_EJECUTAR.md`
- `docs/MAPA_DEL_REPO.md`
